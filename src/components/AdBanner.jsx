...
const AdBanner = ({ title, description, image, link, type = 'horizontal' }) => {
    return (
        <div className={`relative group overflow-hidden rounded-2xl border border-slate-200 bg-white transition-all hover:shadow-lg ${type === 'horizontal' ? 'flex flex-col md:flex-row min-h-[120px]' : 'flex flex-col'}`}>
            {/* Etiqueta de "Anuncio" */}
            ...
            <div className={`${type === 'horizontal' ? 'w-full md:w-1/3' : 'w-full aspect-[16/9]'} overflow-hidden bg-slate-100`}>
                <img
                    src={image || 'https://images.unsplash.com/photo-1621252179027-94459d278660?auto=format&fit=crop&q=80&w=800'}
                    alt={title}
            ...
            <div className={`p-5 flex-1 flex flex-col justify-center ${type === 'horizontal' ? 'md:pl-8' : ''}`}>
                <h4 className="text-lg font-bold text-slate-900 mb-1 group-hover:text-teal-600 transition-colors uppercase tracking-tight">
                    {title}
                </h4>
